...
export default function Notice() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/notice/add`, {
        method: "POST",
      ...
      if (res.ok) {
        alert("Notice added successfully!"); // ⭐ SUCCESS MESSAGE
        setNewNotice("");
        loadNotices();
      }
    } catch {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className={`transition-all duration-300 ${collapsed ? "lg:pl-20" : "lg:pl-64"}`}>
        ...
        <main className="p-6 space-y-8">
          ...
          {/* Create Notice Card */}
          <Card className="glass-card max-w-3xl border border-primary/10 shadow-xl">
            <CardHeader>
              <CardTitle className="text-xl font-semibold">Create Notice</CardTitle>
            </CardHeader>
          ...
          <Card className="glass-card max-w-3xl border border-primary/10 shadow-xl">
            <CardHeader>
              <CardTitle className="text-xl font-semibold">All Notices</CardTitle>
            </CardHeader>
              ...
              <ul className="space-y-3 mt-3">
                {notices.map((n) => (
                  <li
                    ...
                    p-4 shadow-sm flex justify-between"
                  >
                    <span className="text-sm">{n.text}</span>
                    <small className="text-primary font-medium">{n.time}</small>
                  </li>
                ))}
              </ul>
            </CardContent>
